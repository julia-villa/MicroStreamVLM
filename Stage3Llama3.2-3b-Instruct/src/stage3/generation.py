...
from src.stage3.dataset import Stage3SegmentRecord, limit_records
...
def _build_generation_prefix(sample: Stage3SegmentRecord, model) -> list[int]:
    bos_ids = [model.tokenizer.bos_token_id] if model.tokenizer.bos_token_id is not None else []
    system_ids = _encode_text(model.tokenizer, f"{sample.system_prompt.strip()}\n")
    return bos_ids + system_ids + [model.timeline_token_ids["next"]]
...
def _load_generation_inputs(
    ...
    prefix_ids = _build_generation_prefix(sample, model)
...
def generate_segment_prediction(
    ...
    soft_feedback_token_budget: int = 24,
    ...
    while generated_token_count < max_total_new_tokens:
        ...
        if feedback_mode:
            feedback_blocked_ids = _feedback_blocked_token_ids(model)
            ...
            token_biases = (
                ...
                if curr_response_len >= soft_feedback_token_budget
                else None
            )
            ...
            if current_feedback_stop_event is not None:
                ...
                current_feedback_stop_event["no_repeat_ngram_block_applied"] = bool(
                    current_feedback_stop_event["no_repeat_ngram_block_applied"] or forbidden_ngram_tokens
                )
        ...
        trace.append(
            {
                ...
                "feedback_mode": feedback_mode,
                "soft_end_bias_active": bool(feedback_mode and curr_response_len >= soft_feedback_token_budget),
...
def save_predictions(predictions: Sequence[dict[str, Any]], output_path: str | Path) -> None:
    """Persist benchmark predictions to JSON."""
...
def load_predictions(predictions: str | Path | Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    """Load prediction records from a JSON file or an in-memory sequence."""
