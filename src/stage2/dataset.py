...
def _load_json(json_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        while index < text_len:
            while index < text_len and text[index] in {" ", "\t", "\r", "\n", ",", "[", "]"}:
                index += 1
            if index >= text_len:
                break
            ...
            index = end
        if not items:
            raise ValueError(f"Could not parse JSON records from {json_path}") from exc
        data = items
...
class Stage2QEVDFit300KDataset(Dataset):
    ...
    def _expand_feedback_samples(self) -> list[Stage2Sample]:
        ...
        for item in entries:
            ...
            feedback_entries = item.get("feedback")
            if feedback_entries is None:
                feedback_entries = item.get("feedbacks", [])

            for feedback_index, feedback_text in enumerate(feedback_entries):
