...
class FrozenEfficientNetStreamEncoder(nn.Module):
    ...
    def _iter_sampled_frame_items(
        ...
        if timestamps_path is not None:
            source_timestamps = load_video_timestamps(str(timestamps_path))

        source_fps = capture.get(cv2.CAP_PROP_FPS)
        if source_fps is None or source_fps <= 0:
            source_fps = float(self.fps_out)

        next_frame_timestamp = start_timestamp_sec
        frame_index = -1
        try:
            while True:
                ...
                if source_timestamps is not None:
                    if frame_index >= len(source_timestamps):
                        break
                    timestamp_sec = float(source_timestamps[frame_index])
                else:
                    timestamp_sec = frame_index / source_fps

                if start_timestamp_sec is not None and timestamp_sec < start_timestamp_sec:
                    continue
                if end_timestamp_sec is not None and timestamp_sec > end_timestamp_sec:
                    break
                if next_frame_timestamp is None:
                    next_frame_timestamp = timestamp_sec
    ...
    def _iter_sampled_frames(
        ...
        for frame, _ in self._iter_sampled_frame_items(
    ...
    def warmup_stream(self, frame: np.ndarray, rotate_90_cw: bool = False) -> None:
        """Prime the stream state from a single live frame."""
        self.reset_stream_state()
        processed_frame = _resize_for_vertical_pipeline(frame)
        if rotate_90_cw:
            processed_frame = _rotate_and_square_pad(processed_frame)
        for _ in range(self.warmup_frames):
            self._consume_frame(processed_frame, capture_output=False)
        self._stream_is_warmed = True
    ...
    def encode_stream_frame(
        ...
        """Encode one live-stream frame and return one formatted feature step when ready."""
        if not self._stream_is_warmed:
            self.warmup_stream(frame, rotate_90_cw=rotate_90_cw)

        processed_frame = _resize_for_vertical_pipeline(frame)
        if rotate_90_cw:
            processed_frame = _rotate_and_square_pad(processed_frame)

        feature_maps = self._consume_frame(processed_frame, capture_output=True)
        if feature_maps is None:
            return None
        return self._format_feature_maps(feature_maps).cpu()[0]
    ...
    def encode_segment(
        ...
        rotate_90_cw: bool = False,
    ) -> dict[str, torch.Tensor | list[int] | list[float]]:
        """Encode one timestamp-bounded video segment into cached xattn features."""
        sampled_items = list(
            self._iter_sampled_frame_items(
                video_path=video_path,
                rotate_90_cw=rotate_90_cw,
        ...
        if not sampled_items:
            raise ValueError(
                f"No frames sampled for segment {video_path} in [{start_timestamp_sec}, {end_timestamp_sec}]"
            )

        self.reset_stream_state()
        warmup_frame = sampled_items[0][0]
        for _ in range(self.warmup_frames):
            self._consume_frame(warmup_frame, capture_output=False)

        feature_steps: list[torch.Tensor] = []
        feature_timestamps: list[float] = []
        for frame, timestamp_sec in sampled_items:
            step_features = self._consume_frame(frame, capture_output=True)
            if step_features is not None:
                feature_steps.append(step_features.cpu())
                feature_timestamps.append(float(timestamp_sec - start_timestamp_sec))

        if not feature_steps:
            last_frame, last_timestamp = sampled_items[-1]
            while not feature_steps:
                step_features = self._consume_frame(last_frame, capture_output=True)
                if step_features is not None:
                    feature_steps.append(step_features.cpu())
                    feature_timestamps.append(float(last_timestamp - start_timestamp_sec))
